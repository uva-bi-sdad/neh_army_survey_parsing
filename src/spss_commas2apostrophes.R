library(haven)
library(stringr)
file_paths <- list.files("data/working", pattern = "*CLEAN_NOM_nocounts.sav", full.names = T)
...
for (file_path in file_paths) {
  ...
  haven::write_sav(tbl_haven_labelled, paste0(tools::file_path_sans_ext(file_path), "_apos.sav"))
}
